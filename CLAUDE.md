...
**Schema-driven menu layout.** As of v1.2 the renderer walks the API response generically rather than hardcoding `five_hour` / `seven_day` / `extra_usage`. Top-level constants control the layout: `SECTIONS` defines the four sections (`session`, `weekly`, `additional`, `extras`) and their render order; `WEEKLY_SLOTS` and `ADDITIONAL_SLOTS` set how many dynamic rows are pre-allocated under each (8 and 4); `LABELS` is the curated map from API key → `(section, label)`; `SECTION_TITLES` provides the human-readable section headers. Three small helpers — `prettify`, `classify`, `bucket_shape` — are pure functions used by `_update_menu`. To add a curated label for a new Anthropic codename, add an entry to `LABELS`. Everything else (heuristics for unknown keys, fall-through to `prettify(key)`, hiding unused slots and empty section headers) happens automatically.

**Settings persistence.** `~/Library/Application Support/claude-bar/settings.json` (mode 0600) is the source of truth for user preferences across launches: `browser`, `org_id`, `org_name`, `auto_poll_enabled`, `poll_interval_secs`, `show_summary`, plus a `first_run_completed` gate that controls the wizard. `load_settings()` is resilient to missing/corrupt files (falls back to `DEFAULT_SETTINGS` and logs a warning). `save_settings()` is also non-fatal on failure. Every callback that mutates a user preference calls `self._save()`.

**First-run wizard (`_run_first_run_wizard`).** Triggered from a one-shot `rumps.Timer` (~0.3s after launch) when no browser is set in CLI args or settings. Probes every browser in `BROWSERS` via `_validate_browser` (tries to fetch `/api/organizations`), then prompts via `rumps.alert` only when there's ambiguity (2-3 valid browsers → picker; 4+ → silent priority pick + informational alert; 0 valid → "log in first" alert and bail without marking complete). Org selection uses the same priority logic as the rest of the app — paid plans first.

**Organization switcher (`_rebuild_org_menu`).** Submenu populated from `fetch_organizations(...)` (called from the wizard or from `_ensure_session` for users with saved settings). Cached on `self._orgs`. The checkmark reflects `self._org_id`. Clicking an item invokes `_switch_org(uuid, name)` which persists, repaints, and triggers a refresh. From background threads, route through `callAfter(self._rebuild_org_menu, orgs)`.

**Browser switcher (`_repaint_browser_marks` / `_switch_browser`).** Static submenu populated from `BROWSERS` at construction. Click → invalidates the session and orgs caches, persists, refreshes. CLI `--browser` is a launch-time hint that gets superseded if the user clicks a browser in the submenu.
...
- **`settings.json` is the source of truth for user prefs across launches.** `_init_state` reads from `load_settings()`; every callback that mutates state writes back via `self._save()`. CLI `--browser` and `--org` are session-only overrides that don't rewrite settings. New persisted prefs go in `DEFAULT_SETTINGS` so old files merge correctly via `{**DEFAULT_SETTINGS, **loaded}`.
- **Default behavior on a fresh install:** auto-poll OFF, interval 10 min, no browser/org chosen → first-run wizard. After the wizard or a CLI flag, those choices persist.
- **Initial fetch on launch is intentional** (`_refresh(None)` in `__init__`) — but gated on whether we have a known browser. If neither CLI nor settings have a browser, the wizard runs first and triggers the refresh after picking. Don't move this fetch behind the auto-poll toggle.
- **Section row slots are pre-allocated** at menu construction time (`WEEKLY_SLOTS=8`, `ADDITIONAL_SLOTS=4`). The renderer fills the first N and hides the rest. If Anthropic returns more buckets than fit, you'll see `[claude_bar] no slot for ...` in the log — bump the constant.
- **Menu mutation must happen on the main thread.** Background threads (`_refresh_bg`, etc.) use `callAfter(...)` to schedule menu repaints. The Org submenu rebuild is the most common case (`callAfter(self._rebuild_org_menu, orgs)` from `_ensure_session`).
